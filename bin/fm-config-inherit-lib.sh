...
# spawn on codex too, primary config/crew-model pins the model those crewmates
# launch on, primary config/backlog-backend=manual makes that home
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness crew-model backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context}"
