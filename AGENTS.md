...
config/crew-model  standing crewmate/scout model; LOCAL, gitignored; absent, blank, or "default" = the harness's own default model; applies when the standing harness also governs the spawn, and an explicit --model always wins (section 4). Inherited by secondmate homes
...
config/tavily.env  optional API key giving spawned crewmates and scouts Tavily web retrieval on the harnesses that can also withhold its Research endpoint; LOCAL, gitignored, and NOT inherited by secondmate homes; see docs/configuration.md "Tavily web retrieval"
...
config/gsc.env  optional read-only Google Search Console credential for bin/fm-gsc-pull.sh; LOCAL, gitignored; absent means the pull is simply absent, with nothing warned or degraded; see docs/configuration.md "Search Console pull"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/lesson.md     ship task lesson answer, written by the crewmate before it stops; required by teardown
...
5. **Fleet-state digest** - after that read-once contract and ahead of the context digest, the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the `state/.afk` flag; and one cheap alive/dead read of each task's recorded backend endpoint.
   ...
   A bounded `RECURRING JOBS` section follows, one line per recurring job with its running, due, or upcoming state from `bin/fm-jobs.sh`.
...
Use `gh-axi` for GitHub and `lavish-axi` for structured decisions or reports; consult current help rather than memorizing flags.
For browser work use the harness's own browser tooling (Codex's native browser and computer-use tools, Claude's claude-in-chrome tools), not `chrome-devtools-axi`, while the `fm-chrome-devtools-attach-chrome152` defect is open; the crewmate brief scaffold in `bin/fm-brief.sh` carries the same rule to workers.
...
`config/crew-model` is the standing crewmate model on the same static path; an explicit per-task `--model` always wins.
...
Resolve the task's base ref at intake and pass it explicitly: the right base differs by task, a project's default branch can be stale, and `fm-spawn` refuses rather than assuming one.
...
On a repository where nothing runs a check on a pull request, `bin/fm-dod-lib.sh` owns the evidence test and the run-start CI skip that ready signal depends on.
...
A ship task also owes one recorded lesson before cleanup, written by its worker while the detail is still fresh; `bin/fm-dod-lib.sh` owns that requirement and `bin/fm-teardown.sh` refuses without it, naming the record.
...
The promoted worker must inventory scratch state, return to a clean checkout of the base ref the task was dispatched from, carry over only intended fix changes, create the ship branch, and follow the project's selected delivery path while leaving scratch commits and debug edits behind and turning a reproduced bug into the regression test.
...
A recurring job is a backlog item under the `recurring: ` convention that `docs/configuration.md` "Recurring jobs" owns; `bin/fm-jobs.sh` lists every job with its state at session start and records a completion with its `mark` subcommand.
