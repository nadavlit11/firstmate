...
For a raw `finished` result, the adapter also reads the v3 build-actions endpoint, so any failed action - whatever its phase - is reported as `post-processing-failed` carrying that action's real type in `failed_action`, without losing `raw_status: finished`.
