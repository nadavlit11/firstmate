...
description: >-
  ...
  Use whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, CREW_DISPATCH invalid, SECONDMATE_HARNESS invalid, FLEET_SYNC, NETWORK_CHECKS, HOME_SUMMARY, BACKLOG_RECONCILE, SECONDMATE_SYNC, SECONDMATE_LIVENESS, SECONDMATE_HANDOFF, NUDGE_SECONDMATES, or FMX - or reports that an interrupted backlog cleanup may have left an endpoint or local copy, or when a standalone bin/fm-bootstrap.sh or bin/fm-startup-network.sh run prints one of those lines.
...
- `SECONDMATE_HARNESS: invalid config/secondmate-harness - <reason>` - the secondmate pin carries an effort token that is not `low`; standing configuration cannot authorize higher effort, so every local and remote secondmate spawn refuses until the token is removed. Correct the file rather than working around it; a genuinely needed higher level is a per-spawn exception the captain authorizes on the command line.
- `SECONDMATE_HARNESS: warning the resolved secondmate harness is '<harness>', which has no verified low-effort launch axis` - the pin is legal but no spawn selecting it can prove the level it runs at, so it is refused unless that invocation carries `--effort-override-reason`, or the mate's own record already carries one from an earlier exception. Either pin a harness that can enforce low, or respawn once with a written exception so the recorded reason keeps the mate recoverable.
