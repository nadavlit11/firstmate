...
Google reporting no incomplete date is not Google saying everything is settled: in that case the trailing three days of the documented settling window are assumed incomplete instead, `manifest.firstIncompleteDate` stays null, `firstIncompleteDateSource` reads `assumed-conservative-default` rather than `reported`, `provisionalFromDate` records the boundary that actually gated the days, and the assumption is stated on stderr.
