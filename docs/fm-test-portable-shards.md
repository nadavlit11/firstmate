...
The branch-default regression test added by the same pull request is in a portable parallel lane, not shard 4, and the branch-default export adds no per-test work, so the timeout is stale balance rather than a regression in that fix.

The single longest script, `tests/fm-watch-triage.test.sh` at 372097 ms, is the floor for any shard count.
