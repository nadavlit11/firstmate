...
Git fixtures assume `main`: `bin/fm-test-run.sh` exports an `init.defaultBranch=main` pin for every script it runs, so a bare `git init` fixture is on `main` whatever the host default is - run suites through the runner rather than `bash tests/<x>.test.sh`, and do not re-pin the branch at individual call sites.
