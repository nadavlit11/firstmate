...
A pinned harness with no verified low-effort launch axis (`opencode`, `kimi`, `cursor`) is reported at session start as `SECONDMATE_HARNESS: warning ...` rather than refused: every spawn selecting it needs `--effort-override-reason` naming the capability gap, and once such a spawn has recorded that reason the mate's own record carries it so a liveness respawn or relaunch stays automatic.
