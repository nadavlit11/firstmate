...
phase_recovery() {
  ...
  PATH="$FAKEBIN:$PATH" FM_HOME="$HOME_DIR" FM_FAKE_TMUX_LOG="$LOG" FM_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/fm-spawn.sh" design "echo relaunch" --secondmate \
    --effort-override-reason "echo stands in for a launcher here; a raw command has no low-effort axis to prove" \
    >/dev/null 2>&1 \
