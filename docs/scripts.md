...
| `fm-jobs.sh`             | List every `recurring: ` backlog job with its running/due/upcoming state and record a completion with `mark` |
...
| `fm-procevent-codemagic.sh` | Watch one Codemagic build through the read-only v3 API and wake with its terminal outcome or lookup failure |
