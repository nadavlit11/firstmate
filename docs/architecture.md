...
It also owns the lesson every ship task owes before its worker stops, rendered into the same two artifacts, because the worker is the only holder of that detail once teardown removes it.
...
After that landing safety passes, a ship cleanup also refuses when the task recorded no lesson answer, so the record survives the worker; a task briefed before the requirement existed warns once and proceeds.
